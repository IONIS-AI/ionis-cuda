 ...
 * and writes results back to the destination table (wspr.silver, dropped 2026-09-22).
 *
...
void print_usage(const char* prog) {
    ...
    printf("  -w, --write           Write embeddings to the destination table (currently none)\n");
...
int main(int argc, char* argv[]) {
    ...
    // -------------------------------------------------------------------------
    if (write_back) {
        printf("[6/6] Writing embeddings to the destination table...\n");
        ...
        if (inserted == 0) {
            fprintf(stderr, RED "ERROR: %s\n" RESET, loader.last_error().c_str());
            printf("      " YELLOW "SKIPPED" RESET " (no destination table - wspr.silver was dropped 2026-09-22)\n\n");
        } else {
